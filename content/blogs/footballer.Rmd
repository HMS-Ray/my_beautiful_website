...
Two wonderful players couldn't be differentiated by statistics but ronaldo does have a higher mean success rate. So in conclusion, Messi does score more goals per 90 mins and has better shooting skills but Ronaldo also seems to have a stronger mentality when faced with difficulty. Messi 2:1 Ronaldo.
...
footballer %>% filter(age>=21 & age<=32,season!="2021-2022") %>% 
  group_by(name,age) %>% 
  mutate(total_assists=90*sum(assist)/sum(minutes)) %>% select(name,age,total_assists) %>% unique() %>% ggplot(aes(x=age,y=total_assists,color=name))+geom_point()+geom_smooth(se=FALSE)+theme_bw()
```
